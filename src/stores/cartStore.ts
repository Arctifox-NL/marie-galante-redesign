...
async function createShopifyCart(
  ...
  const data: any = await storefrontApiRequest(CART_CREATE_MUTATION, {
...
async function addLineToShopifyCart(
  ...
  const data: any = await storefrontApiRequest(CART_LINES_ADD_MUTATION, {
    cartId,
...
async function updateShopifyCartLine(
  ...
  const data: any = await storefrontApiRequest(CART_LINES_UPDATE_MUTATION, {
    cartId,
...
async function removeLineFromShopifyCart(
  ...
  const data: any = await storefrontApiRequest(CART_LINES_REMOVE_MUTATION, {
    cartId,
    lineIds: [lineId],
  });
  ...
  persist(
    (set, get) => ({
      ...
      syncCart: async () => {
        ...
        set({ isSyncing: true });
        try {
          const data: any = await storefrontApiRequest(CART_QUERY, { id: cartId });
          if (!data) return;
