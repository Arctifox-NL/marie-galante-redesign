...
const options: Option[] = [
  { key: "oostzee", img: zeilendImg, ctaHref: "/meevaren-oostzee", internal: true },
  { key: "dag", img: dagtochtenImg, ctaHref: "/dagtochten", internal: true },
  {
    key: "bnb",
  ...
  { key: "feest", img: feestjeImg, ctaHref: "mailto:info@marie-galante.nl" },
];
    ...
    <SiteLayout>
      <section className="relative h-[60vh] min-h-[420px] w-full overflow-hidden">
        ...
        <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col justify-end px-6 pb-16 md:px-10 md:pb-24">
          <div className="eyebrow text-background/70">{t("boek.heroEyebrow")}</div>
          <h1 className="mt-3 max-w-3xl font-display text-5xl text-background md:text-7xl">{t("boek.heroTitle")}</h1>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl space-y-28 px-6 pb-32 md:px-10 md:space-y-40">
        {options.map((o, i) => {
          const title = t(`boek.options.${o.key}.title`);
          const eyebrow = t(`boek.options.${o.key}.eyebrow`);
          const body = t(`boek.options.${o.key}.body`);
          const cta = t(`boek.options.${o.key}.cta`);
          return (
          <article key={o.key} className="grid items-center gap-12 md:grid-cols-12 md:gap-20">
            <div className={`md:col-span-6 ${i % 2 ? "md:order-2" : ""}`}>
              {o.collage ? (
                ...
                <img src={o.img} alt={title} className="aspect-[4/3] w-full object-cover" />
              )}
            </div>
            <div className={`md:col-span-6 ${i % 2 ? "md:order-1" : ""}`}>
              ...
              <p className="mt-6 text-lg leading-relaxed text-foreground/80">{body}</p>
              {o.internal ? (
      ...
      <section className="bg-primary py-24 text-primary-foreground">
        <div className="mx-auto max-w-3xl px-6 text-center md:px-10">
          <div className="eyebrow text-primary-foreground/60">{t("boek.ctaEyebrow")}</div>
          <h2 className="mt-4 font-display text-4xl md:text-5xl">{t("boek.ctaTitle")}</h2>
